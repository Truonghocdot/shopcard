...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <!-- Breadcrumb -->
        <nav
            class="flex items-center mb-8 text-[10px] font-black text-text-muted overflow-x-auto whitespace-nowrap pb-2 uppercase tracking-[0.2em]">
            <a class="hover:text-primary inline-flex items-center gap-2 leading-none transition-colors"
                href="{{ route('home') }}">
            ...
            <a class="hover:text-primary inline-flex items-center leading-none transition-colors font-black text-text-secondary"
                href="{{ route('products.index', ['category' => $product->category_id]) }}">{{ $product->category->title }}</a>
            <span class="mx-3 text-white/10">/</span>
            <span
                class="inline-flex items-center leading-none text-primary font-black drop-shadow-[0_0_8px_rgba(230,46,107,0.4)]">{{ $product->title }}</span>
        </nav>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-7 space-y-6">
                <div class="bg-bg-card rounded-2xl overflow-hidden shadow-2xl border border-border relative">
                    <div class="relative group z-10" id="product-carousel">
                        <div id="carousel-slides"
                            class="flex overflow-x-auto snap-x snap-mandatory scroll-smooth p-0 no-scrollbar"
                        ...
                        @if (!empty($product->images) && count($product->images) > 1)
                            <button onclick="moveSlide(-1)"
                                class="absolute left-4 top-1/2 -translate-y-1/2 bg-bg-dark/80 hover:bg-primary text-text-primary p-3 rounded-full opacity-0 group-hover:opacity-100 transition-all shadow-lg z-20 backdrop-blur-md border border-border">
                                <span class="material-icons">chevron_left</span>
                            </button>
                            <button onclick="moveSlide(1)"
                                class="absolute right-4 top-1/2 -translate-y-1/2 bg-bg-dark/80 hover:bg-primary text-text-primary p-3 rounded-full opacity-0 group-hover:opacity-100 transition-all shadow-lg z-20 backdrop-blur-md border border-border">
                                <span class="material-icons">chevron_right</span>
                            </button>
                            <div
                                class="absolute bottom-4 right-4 bg-bg-dark/80 backdrop-blur-md text-text-primary text-[10px] font-black px-4 py-1.5 rounded-full z-20 border border-border uppercase tracking-widest">
                            ...
                            <div class="flex gap-3 overflow-x-auto pb-2 no-scrollbar scroll-smooth"
                                ...
                                @foreach ($product->images as $index => $image)
                                    <button type="button" onclick="scrollToSlide({{ $index }})"
                                        ...
                                        data-index="{{ $index }}">
                                        <img src="{{ url('storage/' . $image) }}" alt="Thumbnail {{ $index + 1 }}"
                                            class="w-full h-full object-cover">
                                    </button>
                                @endforeach
                            </div>
                ...
                <div class="bg-bg-card rounded-2xl p-8 shadow-2xl border border-border relative overflow-hidden">
                    <h1
                        class="text-xl md:text-3xl font-black mb-6 text-text-primary uppercase tracking-tight relative z-20 leading-tight flex items-center gap-3">
                        <span class="material-icons text-primary">shopping_bag</span>
                        {{ $product->title }}
                    </h1>

                    <div
                        class="flex flex-wrap items-center justify-between gap-8 py-8 border-y border-border relative z-20">
                        <div class="space-y-2">
                            @if ($product->sell_price && $product->sell_price > $product->getFinalPrice())
                                <span
                                    class="text-text-muted line-through text-lg font-bold">{{ number_format($product->sell_price) }}đ</span>
                            @endif
                            <div class="flex items-baseline gap-3">
                                <span
                                    class="text-3xl md:text-5xl font-black text-text-primary drop-shadow-[0_0_15px_rgba(230,46,107,0.4)]">{{ number_format($product->getFinalPrice()) }}
                                    <span class="text-lg">đ</span></span>
                                @if ($product->getDiscountPercent())
                                    <span
                                        class="bg-primary text-white text-[10px] md:text-xs font-black px-3 py-1 rounded-full shadow-[0_0_10px_rgba(230,46,107,0.4)] uppercase tracking-widest">{{ __('super_sale') }}</span>
                                @endif
                            </div>
                        </div>
                        <div class="flex flex-col items-end w-full sm:w-auto">
                            @if ($product->sell_price && $product->sell_price > $product->getFinalPrice())
                                <span
                                    class="text-xs text-indigo-400 font-black flex items-center gap-1 mb-4 uppercase tracking-widest">
                                    <span class="material-icons text-sm">savings</span> {{ __('savings') }}
                                    {{ number_format($product->sell_price - $product->getFinalPrice()) }}đ
                                </span>
                            @endif
                            @auth
                            ...
                            @else
                                <a href="{{ route('login') }}"
                                    class="w-full sm:w-auto btn-esport py-5 px-12 rounded-2xl flex items-center justify-center gap-4 uppercase tracking-widest text-base font-black border-none group">
            ...
            <div class="lg:col-span-5 space-y-4" x-data="{ activeTab: 1 }">
                ...
                <div class="bg-bg-card rounded-2xl border border-border overflow-hidden shadow-xl transition-all">
                    <button @click="activeTab = activeTab === 1 ? null : 1"
                        class="w-full flex items-center justify-between p-5 text-left font-black text-sm tracking-widest text-text-primary uppercase hover:bg-white/5 transition-colors">
                        <span class="flex items-center gap-3">
                            <span class="material-icons text-primary">description</span>
                            {{ __('card_details') }}
                        </span>
                        <span class="material-icons transition-transform duration-300"
                            :class="{ 'rotate-180': activeTab === 1 }">expand_more</span>
                    </button>
                    <div x-show="activeTab === 1" x-collapse x-cloak
                        class="border-t border-border p-6 bg-bg-dark/40 font-bold text-sm text-text-secondary leading-relaxed space-y-4">
                ...
                <div class="bg-bg-card rounded-2xl border border-border overflow-hidden shadow-xl transition-all">
                    <button @click="activeTab = activeTab === 2 ? null : 2"
                        ...
                        <span class="material-icons transition-transform duration-300"
                            :class="{ 'rotate-180': activeTab === 2 }">expand_more</span>
                    </button>
                    <div x-show="activeTab === 2" x-collapse x-cloak
                        class="border-t border-border p-6 bg-bg-dark/40 font-bold text-sm text-text-muted leading-relaxed space-y-3">
                ...
                <div class="bg-bg-card rounded-2xl border border-border overflow-hidden shadow-xl transition-all">
                    <button @click="activeTab = activeTab === 3 ? null : 3"
                        ...
                        <span class="material-icons transition-transform duration-300"
                            :class="{ 'rotate-180': activeTab === 3 }">expand_more</span>
                    </button>
                ...
                <div class="bg-bg-card rounded-2xl border border-border overflow-hidden shadow-xl transition-all">
                    <button @click="activeTab = activeTab === 4 ? null : 4"
                        ...
                        <span class="material-icons transition-transform duration-300"
                            :class="{ 'rotate-180': activeTab === 4 }">expand_more</span>
                    </button>
                    <div x-show="activeTab === 4" x-collapse x-cloak
                        ...
                        <p class="flex gap-3 items-start">
                            <span
                                class="w-36 shrink-0 text-primary font-black">{{ __('security_label_condition') }}:</span>
